...
class CopyCommand : ICommand {
  ...
  int arguments() override {
    ...
    parser->addPositionalArgument(
      "SOURCE", "The source file(s) to copy to the destination", "SOURCE..."
    );
  ...
  int command(const QStringList& args) override {
    ...
    if (!toUrl.isLocalFile()) {
      GIO_ERROR(
        toUrl, toPath, "Error while parsing path", "Path is not a local file"
      );
      return EXIT_FAILURE;
    }
    ...
    for (const auto& path : args.mid(0, args.length() - 1)) {
      ...
      if (!url.isLocalFile()) {
        GIO_ERROR(
          url, path, "Error while parsing path", "Path is not a local file"
        );
        failed = true;
        continue;
      }
    ...
    if (
      parser->isSet(preserveOption) && !parser->isSet(defaultPermissionsOption)
    ) {
      cpArgs.append("-p");
    }
...
private:
  ...
  QCommandLineOption preserveOption =
    QCommandLineOption("preserve", "Preserve all attributes of copied files.");
  QCommandLineOption backupOption =
    QCommandLineOption({"b", "backup"}, "NOT IMPLEMENTED");
  QCommandLineOption noDereferenceOption =
    QCommandLineOption({"P", "no-dereference"}, "Never follow symbolic links.");
  QCommandLineOption defaultPermissionsOption = QCommandLineOption(
    "default-permissions",
    "Use the default permissions of the current process for the "
    "destination file, rather than copying the permissions of the source "
    "file."
  );
};
