...
class LaunchCommand : ICommand {
    ...
    int command(const QStringList& args) override {
        ...
        if (reg.isEmpty()) {
            GIO_ERROR(
                url, path, "Invalid application registration", "Invalid JSON"
            );
            return EXIT_FAILURE;
        }
        ...
        if (!reg.contains("displayName")) {
            QFileInfo info(path);
            // Workaround because basename will sometimes return the suffix
            // instead
            reg["displayName"] =
                info.fileName().left(info.fileName().length() - 6);
        }
        ...
        if (std::any_of(
            ...
            for (const auto& error : errors) {
                if (error.level == ErrorLevel::Error ||
                    ...
                    GIO_ERROR(
                        url, path, "Invalid application registration", error
                    )
                }
            }
            return EXIT_FAILURE;
        ...
        if (qpath.path() == "/") {
            GIO_ERROR(
                url,
                path,
                "Error registering transient application",
                "Unable to get apps API"
            );
            return EXIT_FAILURE;
        }
        ...
        if (properties.isEmpty()) {
            GIO_ERROR(
                ...
                path,
                "Error registering transient application",
                "Unable to convert application registration to QVariantMap"
            );
            return EXIT_FAILURE;
        }
        ...
        if (qpath.path() == "/") {
            GIO_ERROR(
                ...
                path,
                "Error registering transient application",
                "Failed to register" << name.toStdString().c_str()
            );
            return EXIT_FAILURE;
        }
