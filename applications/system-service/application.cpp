...
Application::launchNoSecurityCheck()
{
    if (m_process->processId()) {
        ...
        if (sharedSettings.applicationUsage()) {
            transaction =
                Oxide::Sentry::start_transaction("application", "run");
#ifdef SENTRY
        ...
        Oxide::Sentry::sentry_transaction(
            ...
            [this](Oxide::Sentry::Transaction* t) {
                ...
                if (t != nullptr) {
                    sentry_transaction_set_tag(
                        t->inner, "application", name().toStdString().c_str()
                    );
                }
#else
                Q_UNUSED(t);
                ...
                if (p_stdout == nullptr) {
                    ...
                    if (p_stdout_fd < 0) {
                        errno = -p_stdout_fd;
                        O_WARNING(
                            "Failed to create stdout fd:" << -p_stdout_fd
                        );
                    } else {
                ...
                if (p_stderr == nullptr) {
                    p_stderr_fd = sd_journal_stream_fd(
                        name().toStdString().c_str(), LOG_ERR, 1
                    );
                    if (p_stderr_fd < 0) {
                        errno = -p_stderr_fd;
...
Application::pauseNoSecurityCheck(bool startIfNone)
{
    ...
    Oxide::Sentry::sentry_transaction(
        "Pause Application",
        "pause",
        [this, startIfNone](Oxide::Sentry::Transaction* t) {
#ifdef SENTRY
            if (t != nullptr) {
                sentry_transaction_set_tag(
                    t->inner, "application", name().toStdString().c_str()
        ...
        [this](Oxide::Sentry::Transaction* t) {
            ...
            if (!onPause().isEmpty()) {
                Oxide::Sentry::sentry_span(
                    t, "onPause", "Run onPause action", [this]() {
                    ...
                    switch (type()) {
                        ...
                        case Backgroundable:
                            ...
                            if (stateNoSecurityCheck() == Inactive) {
                                ...
                                O_INFO(
                                    "Application took too long to background"
                                    << name()
                                );
...
Application::resumeNoSecurityCheck()
{
    ...
    Oxide::Sentry::sentry_transaction(
        "Resume Application", "resume", [this](Oxide::Sentry::Transaction* t) {
#ifdef SENTRY
            if (t != nullptr) {
                sentry_transaction_set_tag(
                    t->inner, "application", name().toStdString().c_str()
        ...
        [this](Oxide::Sentry::Transaction* t) {
            if (flags().contains("exclusive")) {
                ...
                if (m_backup == nullptr) {
                    ...
                    p.drawImage(
                        frameBuffer->rect(), *m_backup, m_backup->rect()
                    );
                    delete m_backup;
                    m_backup = nullptr;
                }
            ...
            if (!onResume().isEmpty()) {
                Oxide::Sentry::sentry_span(
                    t, "onResume", "Run onResume action", [this]() {
                    ...
                    switch (type()) {
                        ...
                        case Backgroundable:
                            ...
                            if (timer.isValid()) {
                                ...
                                // it continued
                                O_INFO(
                                    "Warning: application took too long to "
                                    "forground"
                                    << name()
                                );
                            } else {
                        ...
                        default:
                            kill(-m_process->processId(), SIGCONT);
                            startSpan(
                                "foreground", "Application is in the foreground"
                            );
                    }
                }
            );
...
Application::stopNoSecurityCheck()
{
    ...
    Oxide::Sentry::sentry_transaction(
        "Stop Application",
        "stop",
        [this, state](Oxide::Sentry::Transaction* t) {
#ifdef SENTRY
            if (t != nullptr) {
                sentry_transaction_set_tag(
                    t->inner, "application", name().toStdString().c_str()
            ...
            if (!onStop().isEmpty()) {
                Oxide::Sentry::sentry_span(
                    t, "onStop", "Run onStop action", [this]() {
            ...
            Oxide::Sentry::sentry_span(t, "stop", "Stop application", [this]() {
                ...
                // Try to wait for the application to stop normally before
                // killing it
                int tries = 0;
...
Application::stateNoSecurityCheck()
{
    switch (m_process->state()) {
        ...
        case QProcess::Running: {
            QFile stat(
                "/proc/" + QString::number(m_process->processId()) + "/stat"
            );
            ...
            if (type() == Background ||
                (type() == Backgroundable && m_backgrounded)) {
                return InBackground;
            }
            return InForeground;
        } break;
...
Application::finished(int exitCode)
{
    ...
    if (exitCode != EXIT_SUCCESS && exitCode != SIGINT && exitCode != SIGTERM &&
        exitCode != SIGKILL && exitCode != 128 + EXIT_SUCCESS &&
        exitCode != 128 + SIGINT && exitCode != 128 + SIGTERM) {
        notificationAPI
            ->add(
                QUuid::createUuid().toString(),
                "codes.eeems.tarnish",
...
Application::errorOccurred(QProcess::ProcessError error)
{
    switch (error) {
        ...
        case QProcess::ReadError:
            O_INFO(
                "Application" << name()
                              << "unable to read from stdout or stderr."
            );
            break;
...
Application::updateEnvironment()
{
    ...
    if (!flags().contains("nopreload") &&
        !flags().contains("nopreload.sysfs")) {
    ...
    if (!flags().contains("nopreload") &&
        !flags().contains("nopreload.compositor")) {
