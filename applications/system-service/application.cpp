  ...
  if (m_process == nullptr || m_process->processId()) {
    ...
    if (sharedSettings.applicationUsage()) {
      ...
      if (transaction != nullptr) {
        sentry_transaction_set_tag(
          transaction->inner, "application", name().toStdString().c_str()
        );
      }
#endif
    ...
    Oxide::Sentry::sentry_transaction(
      ...
      [this, args](Oxide::Sentry::Transaction* t) {
        ...
        if (p_stdout == nullptr) {
          p_stdout_fd =
            sd_journal_stream_fd(name().toStdString().c_str(), LOG_INFO, 1);
          if (p_stdout_fd < 0) {
            errno = -p_stdout_fd;
        ...
        if (p_stderr == nullptr) {
          p_stderr_fd =
            sd_journal_stream_fd(name().toStdString().c_str(), LOG_ERR, 1);
          if (p_stderr_fd < 0) {
            errno = -p_stderr_fd;
...
Application::stopNoSecurityCheck() {
  ...
  Oxide::Sentry::sentry_transaction(
    "Stop Application", "stop", [this, state](Oxide::Sentry::Transaction* t) {
#ifdef SENTRY
      if (t != nullptr) {
        sentry_transaction_set_tag(
          t->inner, "application", name().toStdString().c_str()
...
Application::stateNoSecurityCheck() {
  ...
  switch (m_process->state()) {
    ...
    case QProcess::Running: {
      ...
      if (
        type() == Background || (type() == Backgroundable && m_backgrounded)
      ) {
        return InBackground;
      }
      return InForeground;
...
Application::started() {
  ...
  if (exclusive || system) {
    ...
    while (true) {
      ...
      if (
        !reply.isValid() || reply.value() || stateNoSecurityCheck() == Inactive
      ) {
        break;
      }
      if (waitTimer.elapsed() > 5000) {
        O_WARNING("Timed out waiting for compositor connection" << identifier);
        break;
      }
...
Application::errorOccurred(QProcess::ProcessError error) {
  switch (error) {
    ...
    case QProcess::ReadError:
      O_INFO(
        "Application" << name() << "unable to read from stdout or stderr."
      );
      break;
