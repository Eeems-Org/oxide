...
Wlan::setInterface(QString path) {
  ...
  connect(
    m_interface,
    &Interface::NetworkRemoved,
    this,
    &Wlan::onNetworkRemoved,
    Qt::QueuedConnection
  );
  connect(
    m_interface,
...
bool
Wlan::down() {
  return !system(("/sbin/ifconfig " + iface() + " down").toStdString().c_str());
}
...
Wlan::isConnected() {
  try {
    auto ip = exec(
      ...
      " | /bin/grep default | /usr/bin/awk '{print $3}'"
    );
...
Wlan::link() {
  ...
  try {
    auto out = exec(
      "/bin/grep " + iface() + " /proc/net/wireless | /usr/bin/awk '{print $3}'"
    );
...
Wlan::rssi() {
  ...
  try {
    auto out = exec(
      "/bin/grep " + iface() + " /proc/net/wireless | /usr/bin/awk '{print $4}'"
    );
    if (QString(out.c_str()).isEmpty()) {
      return 0;
    }
