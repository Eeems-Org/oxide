...
#ifdef SENTRY
            sentry_breadcrumb(
                "dbusservice", "Failed to connect to system bus.", "error"
            );
#endif
...
#ifdef SENTRY
            sentry_breadcrumb(
                "dbusservice", "Unable to register service", "error"
            );
#endif
            qFatal(
                "Unable to register service: %s",
                ex.message().toStdString().c_str()
            );
        }
        O_DEBUG("Registering object...");
        if (!bus.registerObject(
                OXIDE_SERVICE_PATH, instance, QDBusConnection::ExportAllContents
            )) {
#ifdef SENTRY
            sentry_breadcrumb(
                "dbusservice", "Unable to register interface", "error"
            );
#endif
            qFatal(
  ...
  : APIBase(parent), apis(), m_exiting{ false }
{
    ...
    if (res > 0) {
        ...
        qInfo() << "Watchdog notification expected every"
                << QString("%1:%2:%3.%4")
                       .arg(hrs.count())
                       .arg(mins.count())
                       .arg(secs.count())
        ...
        qInfo() << "Watchdog scheduled for every "
                << QString("%1:%2:%3.%4")
                       .arg(hrs.count())
                       .arg(mins.count())
                       .arg(secs.count())
                       .arg(ms.count());
        m_watchdogTimer = startTimer(
            duration_cast<milliseconds>(usec).count(), Qt::PreciseTimer
        );
        ...
        qWarning() << "Failed to detect if watchdog timer is expected:"
                   << strerror(-res);
    } else {
    ...
    Oxide::Sentry::sentry_transaction(
        "DBus Service Init", "init", [this](Oxide::Sentry::Transaction* t) {
            Oxide::Sentry::sentry_span(
                t, "apis", "Initialize APIs", [this](Oxide::Sentry::Span* s) {
                    Oxide::Sentry::sentry_span(
                        s, "wifi", "Initialize wifi API", [this] {
                            apis.insert(
                                "wifi",
                                APIEntry{
                                    .path =
                                        QString(OXIDE_SERVICE_PATH) + "/wifi",
                    ...
                    Oxide::Sentry::sentry_span(
                        s, "system", "Initialize system API", [this] {
                            apis.insert(
                                "system",
                                APIEntry{
                                    .path =
                                        QString(OXIDE_SERVICE_PATH) + "/system",
                                    .dependants = new QStringList(),
                    ...
                    Oxide::Sentry::sentry_span(
                        s, "power", "Initialize power API", [this] {
                            apis.insert(
                                "power",
                                APIEntry{
                                    .path =
                                        QString(OXIDE_SERVICE_PATH) + "/power",
                                    .dependants = new QStringList(),
                    ...
                    Oxide::Sentry::sentry_span(
                        s, "screen", "Initialize screen API", [this] {
                            apis.insert(
                                "screen",
                                APIEntry{
                                    .path =
                                        QString(OXIDE_SERVICE_PATH) + "/screen",
                                    .dependants = new QStringList(),
                    ...
                    Oxide::Sentry::sentry_span(
                        s, "apps", "Initialize apps API", [this] {
                            apis.insert(
                                "apps",
                                APIEntry{
                                    .path =
                                        QString(OXIDE_SERVICE_PATH) + "/apps",
                                    .dependants = new QStringList(),
                    ...
                    Oxide::Sentry::sentry_span(
                        ...
                        [this] {
                            apis.insert(
                                "notification",
                                APIEntry{
                                    .path = QString(OXIDE_SERVICE_PATH) +
                                            "/notification",
                                    .dependants = new QStringList(),
...
DBusService::requestAPI(QString name, QDBusMessage message)
{
    ...
    if (bus.objectRegisteredAt(api.path) == nullptr) {
        bus.registerObject(
            api.path, api.instance, QDBusConnection::ExportAllContents
        );
    }
...
DBusService::releaseAPI(QString name, QDBusMessage message)
{
    ...
    if (!api.dependants->size()) {
        ...
        QDBusConnection::systemBus().unregisterObject(
            api.path, QDBusConnection::UnregisterNode
        );
...
)
{
    ...
    if (newOwner.isEmpty()) {
        ...
        for (auto key : apis.keys()) {
            ...
            if (!api.dependants->size() &&
                bus.objectRegisteredAt(api.path) != nullptr) {
