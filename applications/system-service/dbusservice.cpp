  ...
  if (res > 0) {
    ...
    qInfo() << "Watchdog scheduled for every "
            << QString("%1:%2:%3.%4")
                 ...
                 .arg(ms.count());
    m_watchdogTimer =
      startTimer(duration_cast<milliseconds>(usec).count(), Qt::PreciseTimer);
    ...
    "DBus Service Init", "init", [this](Oxide::Sentry::Transaction* t) {
      Oxide::Sentry::sentry_span(
        t, "apis", "Initialize APIs", [this](Oxide::Sentry::Span* s) {
          ...
          Oxide::Sentry::sentry_span(
            s, "notification", "Initialize notification API", [this] {
              apis.insert(
                "notification",
                APIEntry{
...
DBusService::serviceOwnerChanged(
  ...
  if (newOwner.isEmpty()) {
    ...
    for (auto key : apis.keys()) {
      ...
      if (
        !api.dependants->size() && bus.objectRegisteredAt(api.path) != nullptr
      ) {
