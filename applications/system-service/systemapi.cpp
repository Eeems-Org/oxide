    ...
    if (suspending) {
        ...
        Oxide::Sentry::sentry_transaction(
            "Suspend System", "suspend", [this](Oxide::Sentry::Transaction* t) {
                if (autoLock()) {
                    lockTimestamp = QDateTime::currentMSecsSinceEpoch() +
                                    lockTimer.remainingTime();
                ...
                Oxide::Sentry::sentry_span(
                    t, "screen", "Update screen with suspend image", [this] {
                        ...
                        if (rect == img.rect()) {
                            ...
                            painter.translate(
                                0 - img.width() / 2, 0 - img.height() / 2
                            );
                            painter.drawPixmap(
                                img.rect(), QPixmap::fromImage(img)
                            );
                            painter.end();
                        }
                ...
                Oxide::Sentry::sentry_span(
                    t, "prepare", "Prepare for suspend", [this] {
                ...
                Oxide::Sentry::sentry_span(
                    t, "disable", "Disable various services", [this] {
                        ...
                        getCompositorDBus()
                            ...
                            .waitForFinished();
                        if (m_buffer != nullptr) {
                            Blight::connection()->waitForMarker(
                                m_buffer->surface
                            );
                        }
        ...
        Oxide::Sentry::sentry_transaction(
            "Resume System", "resume", [this](Oxide::Sentry::Transaction* t) {
                qDebug() << "Resuming...";
                Oxide::Sentry::sentry_span(
                    t, "inhibit", "Inhibit sleep", [this] { inhibitSleep(); }
                );
                if (m_buffer != nullptr) {
                ...
                Oxide::Sentry::sentry_span(
                    ...
                    [this] {
                        ...
                        if (lockOnSuspend() || lockTimeout) {
                            ...
                            auto lockscreenApp = appsAPI->getApplication(
                                appsAPI->lockscreenApplication()
                            );
                            if (lockscreenApp != nullptr) {
                                O_DEBUG(
                                    "Resume app set to lockscreen application"
                                );
                        ...
                        if (resumeApp == nullptr) {
                            ...
                            resumeApp = appsAPI->getApplication(
                ...
                Oxide::Sentry::sentry_span(
                    t, "enable", "Enable various services", [this] {
                        emit deviceResuming();
                        if (autoSleep() && powerAPI->chargerState() !=
                                               PowerAPI::ChargerConnected) {
        ...
        "System API Init", "init", [this](Oxide::Sentry::Transaction* t) {
            Oxide::Sentry::sentry_span(
                t, "settings", "Sync settings", [this](Oxide::Sentry::Span* s) {
                    ...
                    Oxide::Sentry::sentry_span(
                        s, "sync", "Sync settings", [this] { settings.sync(); }
                    );
                    Oxide::Sentry::sentry_span(
                        s, "singleton", "Instantiate singleton", [this] {
                            singleton(this);
                        }
                    );
                    resumeApp = nullptr;
                ...
                [this](Oxide::Sentry::Span* s) {
                    ...
                    if (QFile::exists(settings.fileName())) {
                        ...
                        if (settings.contains("autoSleep")) {
                            ...
                            sharedSettings.set_autoSleep(
                                settings.value("autoSleep").toInt()
                            );
                        }
                        ...
                        if (size) {
                            ...
                            for (short i = Right; i <= Down && i < size; i++) {
                                ...
                                O_DEBUG(
                                    QString("Importing old swipe[%1]").arg(i)
                                );
                                sharedSettings.setValue(
                                    "enabled", settings.value("enabled", true)
                                );
                                sharedSettings.setValue(
                                    "length", settings.value("length", 30)
                                );
                            }
            ...
            Oxide::Sentry::sentry_span(
                t, "swipes", "Load swipe settings", [this]() {
                    sharedSettings.beginReadArray("swipes");
                    for (short i = Right; i <= Down; i++) {
                        sharedSettings.setArrayIndex(i);
                        swipeStates[(SwipeDirection)i] =
            ...
            Oxide::Sentry::sentry_span(
                ...
                [this](Oxide::Sentry::Span* s) {
                    Oxide::Sentry::sentry_span(
                        s, "inhibitSleep", "Inhibit sleep", [this] {
                            inhibitSleep();
                        }
                    );
                    Oxide::Sentry::sentry_span(
            ...
            Oxide::Sentry::sentry_span(
                t, "input", "Connect input events", [this] {
                    eventListener->append(
...
SystemAPI::uninhibitSleep(QDBusMessage message)
{
    ...
    if (!sleepInhibited() && autoSleep() &&
        ...
        if (!suspendTimer.isActive()) {
            O_DEBUG(
                "Suspend timer re-enabled due to uninhibit sleep"
                << message.service()
            );
            suspendTimer.start(autoSleep() * 60 * 1000);
        }
