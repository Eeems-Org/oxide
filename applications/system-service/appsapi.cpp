        ...
        "Init Apps API", "init", [this](Oxide::Sentry::Transaction* t) {
            Oxide::Sentry::sentry_span(
                ...
                [this](Oxide::Sentry::Span* s) {
                    Oxide::Sentry::sentry_span(
                        s, "singleton", "Setup singleton", [this] {
                            singleton(this);
                        }
                    );
                    Oxide::Sentry::sentry_span(
                    ...
                    if (version < OXIDE_SETTINGS_VERSION) {
                        Oxide::Sentry::sentry_span(
                            ...
                            [this, version] { migrate(&settings, version); }
                        );
                    }
                }
            );
            ...
            Oxide::Sentry::sentry_span(
                ...
                [this](Oxide::Sentry::Span* s) {
                    Oxide::Sentry::sentry_span(
                        ...
                        [this] {
                            auto path = QDBusObjectPath(
                                settings.value("lockscreenApplication")
                                    .toString()
                            );
                    ...
                    Oxide::Sentry::sentry_span(
                        ...
                        [this] {
                            auto path = QDBusObjectPath(
                                settings.value("processManagerApplication")
                                    .toString()
                            );
                            auto app = getApplication(path);
                            if (app == nullptr) {
                    ...
                    Oxide::Sentry::sentry_span(
                        ...
                        [this] {
                            auto path = QDBusObjectPath(
                                settings.value("taskSwitcherApplication")
                                    .toString()
                            );
                            auto app = getApplication(path);
                            if (app == nullptr) {
...
AppsAPI::registerApplicationNoSecurityCheck(QVariantMap properties)
{
    ...
    if (type < ApplicationType::Foreground ||
        type > ApplicationType::Backgroundable) {
    ...
    if (bin.isEmpty() || !QFile::exists(bin)) {
        O_WARNING(
            "Invalid configuration: " << name << " has invalid bin" << bin
        );
        return QDBusObjectPath("/");
    }
    if (!QFileInfo(bin).isExecutable()) {
        O_WARNING(
            "Invalid configuration: "
            << name << " has bin that is not executable" << bin
        );
        return QDBusObjectPath("/");
    }
...
AppsAPI::runningApplicationsNoSecurityCheck()
{
    ...
    for (auto app : applications) {
        ...
        if (state == Application::InForeground ||
            state == Application::InBackground) {
            result.insert(app->name(), QVariant::fromValue(app->qPath()));
        }
    }
    return result;
...
AppsAPI::readApplications()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (type < Foreground || type > Backgroundable || name.isEmpty() ||
            bin.isEmpty()) {
