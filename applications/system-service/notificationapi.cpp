...
NotificationAPI::startup() {
  ...
  m_window->setProperty("WA_WAVEFORM", Blight::WaveformMode::Fast);
...
NotificationAPI::getUnownedNotifications() {
  ...
  QStringList names =
    QDBusConnection::systemBus().interface()->registeredServiceNames();
  for (auto notification : m_notifications.values()) {
...
NotificationAPI::add(
  ...
  auto notification = new Notification(
    getPath(identifier), identifier, owner, application, text, icon, this
  );
...
NotificationAPI::add(
  ...
  auto notification =
    add(identifier, message.service(), application, text, icon);
  if (notification == nullptr) {
    return QDBusObjectPath("/");
  }
  return notification->qPath();
...
NotificationAPI::getPath(QString id) {
  static const QUuid NS =
    QUuid::fromString(QLatin1String("{66acfa80-020f-11eb-adc1-0242ac120002}"));
