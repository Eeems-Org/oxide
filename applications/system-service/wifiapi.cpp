    ...
    Oxide::Sentry::sentry_transaction(
        "Wifi API Init", "init", [this](Oxide::Sentry::Transaction* t) {
            Oxide::Sentry::sentry_span(
                t, "singleton", "Setup singleton", [this] { singleton(this); }
            );
            Oxide::Sentry::sentry_span(
                t,
                "sysfs",
                "Finding wireless devices",
                [this](Oxide::Sentry::Span* s) {
                    ...
                    for (auto path : dir.entryList(
                        ...
                        O_DEBUG(
                            ("  Checking " + path + "...").toStdString().c_str()
                        );
                            ...
                            for (auto wlan : wlans) {
                                Oxide::Sentry::sentry_span(
                                    ...
                                    [this, bus, wlan] {
                                        ...
                                        auto reply =
                                            (QDBusReply<QDBusObjectPath>)
                                                supplicant->GetInterface(iface);
                                        if (!reply.isValid() ||
                                            reply.value().path() == "/") {
                                            QVariantMap args;
                                            args.insert("Ifname", iface);
                                            reply = supplicant->CreateInterface(
                                                args
                                            );
                                        }
                                        if (reply.isValid()) {
            ...
            Oxide::Sentry::sentry_span(
                t, "load", "Load state", [this](Oxide::Sentry::Span* s) {
                    Oxide::Sentry::sentry_span(
                        s, "timer", "Setup timer", [this] {
                            ...
                            connect(
                                ...
                                &QTimer::timeout,
                                this,
                                QOverload<>::of(&WifiAPI::update)
                            );
                        }
                    );
                    Oxide::Sentry::sentry_span(
                        s, "networks", "Load networks from disk", [this] {
                            loadNetworks();
                        }
                    );
                    Oxide::Sentry::sentry_span(
                    ...
                    Oxide::Sentry::sentry_span(
                        s, "update", "Update current state", [this] {
                            update();
                        }
                    );
                    Oxide::Sentry::sentry_span(
                    ...
                    Oxide::Sentry::sentry_span(
                        s, "timer", "Start timer", [this] { timer->start(); }
                    );
                }
            );
        }
...
WifiAPI::addNetwork(QString ssid, QVariantMap properties)
{
    ...
    auto network =
        new Network(getPath("network", ssid), ssid, properties, this);
...
WifiAPI::getNetwork(QVariantMap properties)
{
    ...
    for (auto network : networks) {
        ...
        for (auto key : properties.keys()) {
            ...
            if ((key == "ssid" && value.toString() != network->ssid()) ||
                (key == "protococl" &&
                 value.toString() != network->protocol()) ||
    ...
    for (auto registration : registeredNetworks) {
        ...
        for (auto network : networks) {
            if (network->ssid() == ssid && network->protocol() == protocol) {
                ...
                found = true;
                if (network->password() !=
                    registration["password"].toString()) {
        ...
        if (!found) {
            QVariantMap args;
            auto network = new Network(
                getPath("network", ssid), ssid, QVariantMap(), this
            );
