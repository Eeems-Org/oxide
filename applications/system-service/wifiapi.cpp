  ...
  Oxide::Sentry::sentry_transaction(
    "Wifi API Init", "init", [this](Oxide::Sentry::Transaction* t) {
      ...
      Oxide::Sentry::sentry_span(
        t, "sysfs", "Finding wireless devices", [this](Oxide::Sentry::Span* s) {
            ...
            Oxide::Sentry::sentry_span(
              ...
              [this, item] {
                ...
                connect(
                  ...
                  &Wlan::NetworkRemoved,
                  this,
                  &WifiAPI::NetworkRemoved,
                  Qt::QueuedConnection
                );
                connect(
                  item,
                  &Wlan::NetworkSelected,
                  this,
                  &WifiAPI::NetworkSelected,
                  Qt::QueuedConnection
                );
                connect(
                  item,
                  &Wlan::PropertiesChanged,
                  this,
                  &WifiAPI::InterfacePropertiesChanged,
                  Qt::QueuedConnection
                );
                connect(
                  item,
      ...
      Oxide::Sentry::sentry_span(
        t,
        "wpa_supplicant",
        "Connect to the wpa_supplicant",
        [this](Oxide::Sentry::Span* s) {
          Oxide::Sentry::Span* span = Oxide::Sentry::start_span(
            s, "connect", "Connect to DBus interface"
          );
          ...
          validateSupplicant();
          supplicant = new Wpa_Supplicant(
            WPA_SUPPLICANT_SERVICE, WPA_SUPPLICANT_SERVICE_PATH, bus
          );
          connect(
            supplicant,
            &Wpa_Supplicant::InterfaceAdded,
            this,
            &WifiAPI::InterfaceAdded
          );
          connect(
            supplicant,
            &Wpa_Supplicant::InterfaceRemoved,
            this,
            &WifiAPI::InterfaceRemoved
          );
          connect(
            supplicant,
            &Wpa_Supplicant::PropertiesChanged,
            this,
            &WifiAPI::PropertiesChanged
          );
          ...
          Oxide::Sentry::sentry_span(
            s,
            "wlans",
            "Setting up wlan instances",
            [this, bus](Oxide::Sentry::Span* s) {
              for (auto wlan : wlans) {
                Oxide::Sentry::sentry_span(
                  s,
                  wlan->iface().toStdString(),
                  "Load current networks and BSSs",
                  [this, bus, wlan] {
                    auto iface = wlan->iface();
                    auto reply = (QDBusReply<QDBusObjectPath>)
                                   supplicant->GetInterface(iface);
                    ...
                    if (reply.isValid()) {
                      ...
                      for (auto path : interface->networks()) {
                        ...
                        if (!found) {
                          INetwork inetwork(
                            WPA_SUPPLICANT_SERVICE, path.path(), bus, interface
                          );
                          auto properties = inetwork.properties();
                          auto network = new Network(
                            getPath("network", properties["ssid"].toString()),
                            properties,
                            this
                          );
      ...
      Oxide::Sentry::sentry_span(
        t, "load", "Load state", [this](Oxide::Sentry::Span* s) {
          Oxide::Sentry::sentry_span(s, "timer", "Setup timer", [this] {
            ...
            connect(
              timer, &QTimer::timeout, this, QOverload<>::of(&WifiAPI::update)
            );
          });
          Oxide::Sentry::sentry_span(
            s, "networks", "Load networks from disk", [this] { loadNetworks(); }
          );
          Oxide::Sentry::sentry_span(
            s, "state", "Syncronize state with settings", [this] {
          ...
          Oxide::Sentry::sentry_span(
            s, "update", "Update current state", [this] { update(); }
          );
          Oxide::Sentry::sentry_span(
            s, "setEnabled", "Enable/disable API objects", [this] {
...
WifiAPI::InterfaceRemoved(const QDBusObjectPath& path) {
  for (auto wlan : wlans) {
    if (
      wlan->interface() != nullptr && wlan->interface()->path() == path.path()
    ) {
      auto interface = wlan->interface();
      for (auto network : networks) {
...
WifiAPI::loadNetworks() {
  ...
  for (auto registration : registeredNetworks) {
    ...
    if (!found) {
      QVariantMap args;
      auto network =
        new Network(getPath("network", ssid), ssid, QVariantMap(), this);
...
WifiAPI::update() {
  ...
  if (
    (state == Online || state == Offline) && m_currentNetwork.path() != path
  ) {
    O_INFO("Connected to" << path);
    m_currentNetwork.setPath(path);
    emit networkConnected(m_currentNetwork);
  } else if (
    (state == Disconnected || state == Off) && m_currentNetwork.path() != "/"
  ) {
...
WifiAPI::getPath(QString type, QString id) {
  static const QUuid NS =
    QUuid::fromString(QLatin1String("{78c28d66-f558-11ea-adc1-0242ac120002}"));
  if (type == "network") {
