...
ScreenAPI::screenshot()
{
    ...
    if (screen == nullptr || screen->size().isEmpty()) {
        ...
        if (systemAPI->landscape()) {
            saved =
                screen->transformed(QTransform().rotate(270.0)).save(filePath);
        } else {
...
ScreenAPI::addScreenshot(QString filePath)
{
    ...
    Oxide::Sentry::sentry_transaction(
        "Add Screenshot",
        "addScreenshot",
        [this, filePath, &instance](Oxide::Sentry::Transaction* t) {
            Oxide::Sentry::sentry_span(
                t,
                "screenshot",
                "Create screenshot",
                [this, filePath, &instance] {
                    auto path = QString(OXIDE_SERVICE_PATH "/screenshots/") +
                                QFileInfo(filePath)
                                    .completeBaseName()
                                    .remove('-')
                                    .remove('.');
            ...
            Oxide::Sentry::sentry_span(
                t, "events", "Connect and emit events", [this, &instance] {
  ...
  : APIBase(parent), m_screenshots(), m_enabled(false)
{
    Oxide::Sentry::sentry_transaction(
        "Screen API Init", "init", [this](Oxide::Sentry::Transaction* t) {
            ...
            Oxide::Sentry::sentry_span(
                t, "mkdirs", "Create screenshots directory", [this] {
            ...
            Oxide::Sentry::sentry_span(
                t, "screenshots", "Load existing screenshots", [this] {
