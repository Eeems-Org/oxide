...
int install(QCommandLineParser& parser) {
    ...
    for (QString path : args) {
        ...
        if (!QFile::copy(path, toPath)) {
            qDebug() << "error: Failed to created"
...
int uninstall(QCommandLineParser& parser) {
    ...
    for (QString path : args) {
        ...
        if (!QFile::exists(toPath)) {
            qDebug() << "success:" << toPath.toStdString().c_str()
                     << "doesn't exist";
            continue;
        }
        if (!QFile::remove(toPath)) {
            failure = true;
            qDebug() << "error: Could not remove"
                     << toPath.toStdString().c_str();
            continue;
        }
