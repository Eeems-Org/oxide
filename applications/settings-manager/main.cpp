...
main(int argc, char* argv[])
{
    ...
    if (action == "get") {
        ...
        parser.addPositionalArgument(
            "arguments",
            "Arguments to pass to the method using the following format: "
            "<QVariant>:<Value>. e.g. QString:Test",
            "[arguments...]"
        );
        parser.parse(app.arguments());
        args = parser.positionalArguments();
    ...
    if (apiName == "settings") {
        if (action == "call") {
            if (args.length() == 3 && args.at(2) == "crash") {
                ...
                sentry_transaction(
                    "Dummy rot transaction", "transaction", [](Transaction* t) {
    ...
    if (action == "get") {
        ...
        if (!api->setProperty(
                property.c_str(), args.at(3).toStdString().c_str()
            )) {
        ...
        if (Oxide::DBusConnect(
                ...
                [](QVariantList args) {
                    QTextStream qStdOut(stdout, QIODevice::WriteOnly);
                    if (args.size() > 1) {
                        qStdOut << toJson(args).toStdString().c_str()
                                << Qt::endl;
                    } else if (args.size() == 1 && !args.first().isNull()) {
                        qStdOut << toJson(args.first()).toStdString().c_str()
                                << Qt::endl;
                    } else {
        ...
        if (args.size() > 0) {
            for (auto arg : args) {
                if (!arg.contains(":")) {
                    qDebug() << "Arguments must be in the following format: "
                                "<QVariant>:<Value>";
#ifdef SENTRY
                ...
                if (typeName == "QDBusObjectPath") {
                    variant = QVariant::fromValue(
                        QDBusObjectPath(variant.toString())
                    );
                } else if (typeName == "QDBusSignature") {
                    variant =
                        QVariant::fromValue(QDBusSignature(variant.toString()));
                }
