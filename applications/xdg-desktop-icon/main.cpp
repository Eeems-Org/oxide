...
install(QCommandLineParser& parser) {
  ...
  parser.addPositionalArgument(
    "file", "Application registration to install.", "FILE"
  );
  ...
  if (
    !QFile::exists(APPS_DIR) &&
    mkdir(APPS_DIR, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)
  ) {
  ...
  if (info.absoluteDir().path() == APPS_DIR) {
    qDebug() << "error: " << path.toStdString().c_str()
  ...
  if (
    std::any_of(
      errors.constBegin(), errors.constEnd(), [](const ValidationError& e) {
  ...
  if (QFile::exists(toPath) && !QFile::remove(toPath)) {
    qDebug() << "error: " << toPath.toStdString().c_str()
...
uninstall(QCommandLineParser& parser) {
  parser.addPositionalArgument(
    "uninstall", "Uninstall one or more application registration.", "uninstall"
  );
  parser.addPositionalArgument(
    "file", "Application registration to uninstall.", "FILE"
  );
...
main(int argc, char* argv[]) {
  ...
  parser.setApplicationDescription(
    "A program to (un)install application registrations"
  );
