...
public:
  ...
  void sort(int column, Qt::SortOrder order = Qt::AscendingOrder) override {
    ...
    std::sort(
      taskItems.begin(),
      taskItems.end(),
      [this, order](TaskItem* a, TaskItem* b) -> bool {
  ...
  int removeAll(TaskItem* taskItem) {
    ...
    while (i.hasNext()) {
      ...
      if (item == taskItem) {
        beginRemoveRows(
          QModelIndex(), taskItems.indexOf(item), taskItems.indexOf(item)
        );
        i.remove();
        delete item;
        endRemoveRows();
  ...
  void reload() {
    ...
    while (i.hasNext()) {
      ...
      beginRemoveRows(
        QModelIndex(), taskItems.indexOf(taskItem), taskItems.indexOf(taskItem)
      );
      i.remove();
