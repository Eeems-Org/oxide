...
QCommandLineOption setKeyOption(
  "set-key",
  "Set the KEY key to the value passed to the next --set-value option. A "
  "matching --set-value option is mandatory.",
  "KEY"
);
QCommandLineOption setValueOption(
  "set-value",
  "Set the key specified with the previous --set-key option to VALUE. A "
  "matching --set-key option is mandatory.",
  "VALUE"
);
QCommandLineOption setNameOption("set-name", "NOT IMPLEMENTED", "NAME");
QCommandLineOption copyNameToGenericNameOption(
  "copy-name-to-generic-name",
  "Copy the value of the Name key to the GenericName key. Note that a "
  "desktop file requires a Name key to be valid, so this option will always "
  "have an effect."
);
QCommandLineOption setGenericNameOption(
  "set-generic-name",
  "Set the generic name (key DisplayName) to GENERIC-NAME. If a generic name "
  "was already set, it will be overridden. Localizations of the old generic "
...
QCommandLineOption
  removeNotShowInOption("remove-not-show-in", "NOT IMPLEMENTED", "ENVIRONMENT");
  ...
  if (parser.isSet(setKeyOption) || parser.isSet(setValueOption)) {
    for (int i = 0; i < options.length(); i++) {
      ...
      if (setValueOption.names().contains(option)) {
        qDebug() << "Option \"--set-value\" used without a prior "
                    "\"--set-key\" option";
        qDebug() << "Run 'desktop-file-edit --help' to see a full list "
      ...
      if (setKeyOption.names().contains(option)) {
        option = options[++i];
        if (!setValueOption.names().contains(option)) {
          qDebug() << "Option \"--set-key\" used without a following "
                      "\"--set-value\" option";
          qDebug() << "Run 'desktop-file-edit --help' to see a full "
...
applyChanges(
  ...
  for (int i = 0; i < options.length(); i++) {
    ...
    if (setGenericNameOption.names().contains(option)) {
      reg["displayname"] =
        parser.values(setGenericNameOption)[genericNameIndex];
      genericNameIndex++;
      continue;
    }
