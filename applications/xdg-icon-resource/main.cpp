...
QCommandLineOption themeOption(
    ...
    "theme. Applications may install icons under multiple themes but should at "
...
QCommandLineOption contextOption(
    ...
    "Other common contexts are actions, devices, emblems, filesystems and "
    "stock.",
    "context",
    "apps"
);
...
int install(QCommandLineParser& parser) {
    ...
    parser.addPositionalArgument(
        "icon-name", "Name to use when installing.", "[icon-name]"
    );
    ...
    QString name =
        args.length() == 2 ? args.last() : QFileInfo(fromPath).baseName();
