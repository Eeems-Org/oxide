...
Connection::pause()
{
    ...
    bool replied = false;
    auto conn = connect(
        signalHandler, &Oxide::SignalHandler::sigUsr2, [&timer, &replied] {
            replied = true;
            timer.invalidate();
        }
    );
...
Connection::resume()
{
    ...
    bool replied = false;
    auto conn = connect(
        signalHandler, &Oxide::SignalHandler::sigUsr1, [&timer, &replied] {
            replied = true;
            timer.invalidate();
        }
    );
...
Connection::processInputEvents()
{
    ...
    while (!m_inputQueue.empty()) {
        ...
        int res = send(
            m_serverInputFd,
            reinterpret_cast<const char*>(&m_inputQueue.front()) +
                m_lastEventOffset,
    ...
    while (true) {
...
#ifndef ACK_DEBUG
        if (message->header.type != Blight::MessageType::Ping &&
        ...
        switch (message->header.type) {
            ...
            case Blight::MessageType::Ack:
                ...
                C_WARNING(
                    "Unexpected ack from client" << message->header.ackid
                );
                break;
            default:
