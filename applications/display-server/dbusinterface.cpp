...
DbusInterface::open(QDBusMessage message)
{
    ...
    if (connection == nullptr) {
        sendErrorReply(
            QDBusError::InternalError, "Unable to create connection"
        );
...
)
{
    ...
    if (!surface->isValid()) {
        sendErrorReply(
            QDBusError::InternalError, "Surface buffer is not valid"
        );
        return 0;
    }
...
)
{
    ...
    if (surface == nullptr) {
        sendErrorReply(
            QDBusError::BadAddress, "Connection or Surface not found"
        );
        return;
    }
    for (auto& flag : flags) {
...
DbusInterface::sortZ()
{
    ...
    if (m_focused != nullptr &&
        (!m_focused->isRunning() || m_focused->isStopped())) {
        setFocus(nullptr);
