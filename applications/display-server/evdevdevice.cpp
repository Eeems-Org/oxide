...
EvDevDevice::id() {
  return QString("%1:%2").arg(
    sys.strProperty("id/vendor").c_str(), sys.strProperty("id/product").c_str()
  );
}
  ...
  Oxide::dispatchToThread(thread(), [this] {
    ...
    do {
      ...
      res = libevdev_next_event(
        dev, sync ? LIBEVDEV_READ_FLAG_SYNC : LIBEVDEV_READ_FLAG_NORMAL, &event
      );
      if (res < 0) {
        break;
      }
    ...
    if (res && res != -EAGAIN) {
      O_WARNING("Failed to read input for " << name() << ":" << strerror(-res));
    }
    notifier->setEnabled(true);
  });
}
...
EvDevDevice::build_flood() {
  ...
  auto num_inst = 4;
  input_event* ev =
    (input_event*)malloc(sizeof(struct input_event) * n * num_inst);
