...
main(int argc, char* argv[])
{
    ...
    QString pid = Oxide::execute(
                      "systemctl",
                      QStringList() << "--no-pager" << "show" << "--property"
                                    << "MainPID" << "--value" << "blight"
    )
                      .trimmed();
    if (pid != "0" &&
        ...
        if (QProcess::execute(
                "systemctl", QStringList() << "stop" << "blight"
            )) {
    ...
    if (!QFile::exists(QString::fromStdString(runPath)) &&
    ...
    if (lock < 0) {
        qDebug() << "Unable to establish lock on" << lockPath
                 << strerror(errno);
        if (!parser.isSet(breakLockOption)) {
            return EXIT_FAILURE;
        }
        qDebug() << "Attempting to stop all other instances of blight"
                 << lockPath;
