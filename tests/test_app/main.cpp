...
main(int argc, char* argv[]) {
  ...
  if (!qEnvironmentVariableIsSet("SKIP_TEST")) {
    ...
    }
    {
      ...
      O_DEBUG(
        clipboard.name.c_str() << QString::fromStdString(clipboard.to_string())
      );
    ...
    {
      auto ack =
        connection->repaint(buffer, 0, 0, geometry.width(), geometry.height());
      if (!ack.has_value()) {
    ...
    if (buffers.front()->surface != buffer->surface) {
      O_WARNING(
        "Buffer surface identifier does not match!" << buffers.front()->surface
      );
      return EXIT_FAILURE;
    }
