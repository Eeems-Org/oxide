  ...
  switch (type) {
    ...
    case QInputDeviceManager::DeviceTypeTouch: {
      ...
      if (
        ioctl(
          fd,
          EVIOCGABS((touchData->singleTouch ? ABS_X : ABS_MT_POSITION_X)),
          &absInfo
        ) >= 0
      ) {
      ...
      if (
        ioctl(
          fd,
          EVIOCGABS((touchData->singleTouch ? ABS_Y : ABS_MT_POSITION_Y)),
          &absInfo
        ) >= 0
      ) {
      ...
      if (
        ioctl(fd, EVIOCGABS(ABS_PRESSURE), &absInfo) >= 0 &&
...
OxideEventHandler::processKeyboardEvent(
  ...
  if (
    !keyboardData->m_locks[1] && qtmods & Qt::KeypadModifier && keycode >= 71 &&
    keycode <= 83 && keycode != 74 && keycode != 78
  ) {
    unicode = 0xffff;
    switch (keycode) {
      case 71: // 7 --> Home
  ...
  // translation
  if (
    qtcode == Qt::Key_Tab && (qtmods & Qt::ShiftModifier) == Qt::ShiftModifier
  ) {
...
addTouchPoint(
  ...
  *combinedStates |= tp.state;
  ...
  tp.normalPosition = QPointF(
    (contact.x - touchData->minX) / qreal(touchData->maxX - touchData->minX),
  ...
  if (event->type == EV_ABS) {
    if (
      ...
      touchData->currentData.pressure =
        qBound(touchData->minPressure, event->value, touchData->maxPressure);
    ...
    for (auto i = touchData->contacts.begin(), end = touchData->contacts.end();
         i != end;
         /*erasing*/) {
      auto it = i++;
      ...
      if (
        !touchData->isTypeB && contact.state == QEventPoint::State::Released &&
    ...
    for (auto it = touchData->lastContacts.begin(),
      ...
      if (touchData->isTypeB) {
        if (
          contact.trackingId != touchData->contacts[key].trackingId &&
          contact.state
        ) {
    ...
    if (
      ...
      for (int i = 0; i < pointCount; ++i) {
        ...
        const qreal wx =
          winRect.left() + tp.normalPosition.x() * (winRect.width() - 1);
        const qreal wy =
          winRect.top() + tp.normalPosition.y() * (winRect.height() - 1);
        ...
        if (tp.area.width() == -1) {
          ...
          tp.area = QRectF(
            0, 0, tp.area.width() * sizeRatio, tp.area.height() * sizeRatio
          );
        }
        ...
        if (!touchData->minPressure && !touchData->maxPressure) {
          ...
          tp.pressure = (tp.pressure - touchData->minPressure) /
...
OxideEventHandler::toQtModifiers(quint16 mod) {
  ...
  if (
    mod & (Modifiers::ModShift | Modifiers::ModShiftL | Modifiers::ModShiftR)
  ) {
    qtmod |= Qt::ShiftModifier;
  }
  if (
    mod & (Modifiers::ModControl | Modifiers::ModCtrlL | Modifiers::ModCtrlR)
  ) {
...
OxideEventHandler::parseTouchParams(const QStringList& parameters) {
  ...
  if (invertx) {
    m_rotate *= QTransform::fromTranslate(0.5, 0.5).scale(-1.0, 1.0).translate(
      -0.5, -0.5
    );
  }
  if (inverty) {
