...
namespace Blight {
  std::optional<clipboard_t> getClipboard(const std::string& name) {
    ...
    auto reply =
      dbus->get_property(BLIGHT_SERVICE, "/", BLIGHT_INTERFACE, name, "ay");
    if (reply->isError()) {
      _WARN(
    ...
    ::size_t size = 0;
    auto res =
      sd_bus_message_read_array(reply->message, 'y', &clipboard, &size);
    if (res < 0) {
      _WARN(
  ...
  int open() {
    ...
    if (reply->isError()) {
      _WARN(
        "[Blight::open()::call_method(...)] Error: %s",
        reply->error_message().c_str()
      );
    ...
    if (!fd.has_value()) {
      _WARN(
        "[Blight::open()::read_value(\"h\")] Error: %s",
        reply->error_message().c_str()
      );
      return reply->return_value;
    }
  ...
  std::optional<shared_buf_t> createBuffer(
    ...
    if (buf->fd == -1) {
      _WARN(
        "[Blight::createBuffer()::memfd_create(...)] Error: %s",
        std::strerror(errno)
      );
      delete buf;
      return {};
  ...
  std::optional<surface_id_t> addSurface(
    ...
    if (reply->isError()) {
      _WARN(
        "[Blight::addSurface()::call_method(...)] Error: %s",
        reply->error_message().c_str()
      );
      return {};
    }
    ...
    if (!identifier.has_value()) {
      _WARN(
        "[Blight::addSurface()::read_value(\"q\")] Error: %s",
        reply->error_message().c_str()
      );
      return {};
    }
  ...
  bool setClipboard(clipboard_t& clipboard) {
    ...
    if (
      ...
      _WARN(
        "[Blight::setClipboard(\"%s\")] Error: Invalid clipboard name",
    ...
    if (res < 0) {
      _WARN(
        "[Blight::setClipboard(\"%s\")::sd_bus_message_new_method_call("
        ")] Error: %s",
        clipboard.name.c_str(),
        std::strerror(-res)
      );
    ...
    if (res < 0) {
      _WARN(
        "[Blight::setClipboard(\"%s\")::sd_bus_message_append()] "
        "Error: %s",
        clipboard.name.c_str(),
        std::strerror(-res)
      );
    ...
    if (res < 0) {
      _WARN(
        "[Blight::setClipboard(\"%s\")::sd_bus_message_open_container()"
        "] Error: %s",
        clipboard.name.c_str(),
        std::strerror(-res)
      );
    ...
    res = sd_bus_message_append_array(
      message, 'y', clipboard.data.get(), clipboard.size
    );
    if (res < 0) {
      _WARN(
        "[Blight::setClipboard(\"%s\")::sd_bus_message_append_array()] "
        "Error: %s",
        clipboard.name.c_str(),
        std::strerror(-res)
      );
    ...
    if (res < 0) {
      _WARN(
        "[Blight::setClipboard(\"%s\")::sd_bus_message_close_container("
        ")] Error: %s",
        clipboard.name.c_str(),
        std::strerror(-res)
      );
  ...
  bool updateClipboard(clipboard_t& clipboard) {
    ...
    if (
      ...
      _WARN(
        "[Blight::setClipboard(\"%s\")::sd_bus_call()] Error: Invalid "
        "clipboard name",
        clipboard.name.c_str()
      );
      errno = EINVAL;
