...
namespace Blight {
    ...
    maybe_ackid_ptr_t Connection::send(
        ...
        if (!Blight::send_blocking(
            ...
            _WARN(
                "Failed to write connection message header: %s",
                std::strerror(errno)
            );
            return {};
        }
        if (size && !Blight::send_blocking(m_fd, data, size)) {
            _WARN(
                "Failed to write connection message data: %s",
                std::strerror(errno)
            );
            return {};
        }
    ...
    std::optional<shared_buf_t> Connection::resize(
        ...
        auto res = Blight::createBuffer(
            buf->x, buf->y, width, height, stride, buf->format
        );
        if (!res.has_value()) {
            return {};
        }
    ...
    maybe_ackid_ptr_t Connection::raise(surface_id_t identifier) {
        ...
        return send(
            MessageType::Raise, (data_t)&identifier, sizeof(surface_id_t)
        );
    }
    ...
    maybe_ackid_ptr_t Connection::lower(surface_id_t identifier) {
        ...
        return send(
            MessageType::Lower, (data_t)&identifier, sizeof(surface_id_t)
        );
    }
    ...
    std::optional<shared_buf_t> Connection::getBuffer(surface_id_t identifier) {
        ...
        if (fd == -1) {
            _WARN(
                "Failed to get buffer for surface %hu: %s",
                identifier,
                std::strerror(errno)
            );
            return {};
        ...
        auto maybe =
            send(MessageType::Info, (data_t)&identifier, sizeof(surface_id_t));
        if (!maybe.has_value()) {
            ::close(fd);
            _WARN(
                "Failed to get info for surface %hu: %s",
                identifier,
                std::strerror(errno)
            );
            return {};
        ...
        if (!ack->data_size) {
            ::close(fd);
            _WARN(
                "Failed to get info for surface %hu: It does not exist",
                identifier
            );
            return {};
        }
    ...
    maybe_ackid_ptr_t Connection::remove(surface_id_t identifier) {
        ...
        return send(
            MessageType::Delete, (data_t)&identifier, sizeof(surface_id_t)
        );
    }
            ...
            switch (message->header.type) {
                case MessageType::Ping: {
                    ...
                    auto maybe = connection->send(
                        MessageType::Ack, nullptr, 0, message->header.ackid
                    );
                    if (!maybe.has_value()) {
