...
logMachineIdError(int error, QString name, QString path)
{
    if (error == -ENOENT) {
        ...
        O_WARNING(
            "Unexpected error code reading " + name + ":" << strerror(error)
        );
    }
}
std::string
...
namespace Oxide::Sentry {
    ...
    void sentry_init(const char* name, char* argv[], bool autoSessionTracking)
    {
        ...
        sentry_options_set_database_path(
            options, "/home/root/.cache/Eeems/sentry"
        );
        ...
        sentry_value_set_by_key(
            user, "id", sentry_value_new_string(machineId())
        );
        ...
        sentry_value_set_by_key(
            device,
            "model",
            sentry_value_new_string(deviceSettings.getDeviceName())
        );
