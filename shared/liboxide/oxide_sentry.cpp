...
#define OXIDE_UID                                                              \
  SD_ID128_MAKE(5a, a5, ca, 39, ee, 0b, 4f, 48, 92, 75, 29, ca, 17, 51, 95, 24)
...
namespace Oxide::Sentry {
  ...
  const char* bootId() {
    ...
    if (ret == EXIT_SUCCESS) {
      ...
      // TODO - eventually replace with the following when supported by
      // the
      //        reMarkable kernel
      // char buf[SD_ID128_STRING_MAX];
  ...
  void sentry_init(const char* name, char* argv[], bool autoSessionTracking) {
    ...
    sentry_options_set_auto_session_tracking(
      options, autoSessionTracking && sharedSettings.telemetry()
    );
    if (initialized) {
      return;
    }
    ...
    sentry_options_set_dsn(
      options,
      "https://a0136a12d63048c5a353c4a1c2d38914@glitchtip.eeems.codes/2"
    );
    ...
    sentry_options_set_release(
      options, (std::string(name) + "@" + APP_VERSION).c_str()
    );
    ::sentry_init(options);
    ...
    sentry_value_set_by_key(
      device, "machine-id", sentry_value_new_string(machineId())
    );
    sentry_value_set_by_key(
      device, "version", sentry_value_new_string(version.c_str())
    );
    sentry_value_set_by_key(
      device, "model", sentry_value_new_string(deviceSettings.getDeviceName())
    );
    ...
    QObject::connect(
      &sharedSettings,
      &SharedSettings::telemetryChanged,
      [name, argv, autoSessionTracking](bool telemetry) {
        Q_UNUSED(telemetry)
    ...
    QObject::connect(
      &sharedSettings,
      &SharedSettings::crashReportChanged,
      [name, argv, autoSessionTracking](bool crashReport) {
  ...
  void sentry_breadcrumb(
    ...
    sentry_value_set_by_key(
      crumb, "category", sentry_value_new_string(category)
    );
  ...
  start_transaction(const std::string& name, const std::string& action) {
    ...
    // Hack to force transactions to be reported even though SAMPLE_RATE is
    // 100%
  ...
  void sentry_transaction(
    ...
    auto scopeGuard =
      qScopeGuard([transaction] { stop_transaction(transaction); });
  ...
  Span* start_span(
    ...
    return new Span(sentry_transaction_start_child(
      transaction->inner, (char*)operation.c_str(), (char*)description.c_str()
    ));
#else
    Q_UNUSED(transaction);
    Q_UNUSED(operation);
  ...
  Span* start_span(
    ...
    return new Span(sentry_span_start_child(
      parent->inner, (char*)operation.c_str(), (char*)description.c_str()
    ));
#else
    Q_UNUSED(parent);
    Q_UNUSED(operation);
  ...
  void sentry_span(
...
#ifdef SENTRY
    if (
      !sharedSettings.telemetry() || transaction == nullptr ||
    ...
    auto scopeGuard = qScopeGuard([span] { stop_span(span); });
    callback(span);
#else
    Q_UNUSED(transaction);
    Q_UNUSED(operation);
...
#ifdef SENTRY
    if (
      !sharedSettings.telemetry() || parent == nullptr ||
