...
#define addError(_level, _msg)                                                 \
  errors.append(ValidationError{.level = _level, .msg = _msg});
    auto contains = [app, &errors](const QString& name) -> bool {
      ...
      addError(
        ErrorLevel::Critical, QString("Key \"%1\" is missing").arg(name)
      );
      return false;
    };
    auto isString =
      [app, &errors, contains](const QString& name, bool required) -> bool {
      ...
      addError(
        required ? ErrorLevel::Error : ErrorLevel::Warning,
        QString("Key \"%1\" must contain a string").arg(name)
      );
      return false;
    };
    auto isArray = [app, &errors, contains](
                     const QString& name, const ErrorLevel& level, bool required
                   ) -> bool {
      if (required && !contains(name)) {
        return false;
      } else if (!required && !app.contains(name)) {
      ...
      addError(level, QString("Key \"%1\" must contain an array").arg(name));
      return false;
    };
    auto isFile = [app, &errors, isString](
                    const QString& name, const ErrorLevel& level, bool required
                  ) -> bool {
      ...
      addError(
        level,
        QString("Value \"%1\" for key \"%2\" is a path that does not exist")
          .arg(str, name)
      );
      return false;
    };
    auto isIcon = [app, &errors, isString](
                    const QString& name, const ErrorLevel& level, bool required
                  ) -> bool {
      if (!isString(name, required)) {
        return false;
      ...
      if (QFile::exists(path)) {
        ...
        addError(
          level,
          QString(
            "Value \"%1\" for key \"%2\" is a path to a file that "
      ...
      if (path.isEmpty()) {
        addError(
          level,
          QString("Value \"%1\" for key \"%2\" is not a valid icon spec")
            .arg(value, name)
        );
        return false;
      }
      if (!QFile::exists(path)) {
        addError(
          level,
          QString(
            "Value \"%1\" for key \"%2\" is a path that does not "
            "exist"
          )
            .arg(value, name)
        );
      ...
      addError(
        level,
        QString(
          "Value \"%1\" for key \"%2\" is an icon spec for an icon "
          "that does not exist"
        )
          .arg(value, name)
      );
  ...
  if (                                                                         \
    ...
    if (type == "background") {
      ...
      addError(
        ErrorLevel::Warning,
        QString(
          "Value \"%1\" for key \"type\" is not valid and will "
    ...
    if (isFile("bin", ErrorLevel::Error, true)) {
      ...
      if (!info.isFile()) {
        addError(
          ErrorLevel::Error,
          QString("Value \"%1\" for key \"bin\" is not a path to a file")
            .arg(bin)
        );
        ...
        addError(
          ErrorLevel::Error,
          QString(
            "Value \"%1\" for key \"bin\" is a path to a file that "
            "is not executable"
          )
            .arg(bin)
        );
    ...
    if (isArray("flags", ErrorLevel::Critical, false)) {
      ...
      for (auto flag : flagsArray) {
        if (!flag.isString()) {
          addError(
            ErrorLevel::Error,
            QString(
              "Value \"%1\" for key \"flags\" contains an entry "
              "that is not a string \"%2\""
            )
        ...
        if (DeprecatedFlags.contains(value)) {
          addError(
            ...
            QString(
              "Value \"%1\" for key \"flags\" contains a "
              "deprecated entry \"%2\""
            )
              .arg(flagsJson, value)
          );
          ...
          addError(
            ...
            QString(
              "Value \"%1\" for key \"flags\" contains an entry "
              "that should only be used by the system \"%2\""
            )
              .arg(flagsJson, value)
          );
      ...
      if (type == "background" && flags.contains("nosavescreen")) {
        addError(
          ErrorLevel::Hint,
          "Key \"flags\" contains \"nosavescreen\" while \"type\" "
          "has value \"background\""
        );
      }
    } else
      ...
      for (int i = 0; i < directories.count(); i++) {
        ...
        if (!entry.isString()) {
          addError(
            ErrorLevel::Error,
            QString(
              "Value \"%1\" for key \"directories[%2]\" contains "
              "an entry that is not a string \"%3\""
            )
              .arg(
                Oxide::JSON::toJson(directories),
                QString::number(i),
                Oxide::JSON::toJson(entry)
              )
          );
          continue;
        }
        ...
        if (!QFile::exists(directory)) {
          addError(
            ...
            QString(
              "Value \"%1\" for key \"directories[%2]\" contains "
              "a path that does not exist \"%3\""
            )
              .arg(
                Oxide::JSON::toJson(directories), QString::number(i), directory
              )
          );
        }
      }
      ...
      for (int i = 0; i < permissions.count(); i++) {
        ...
        if (!entry.isString()) {
          addError(
            ErrorLevel::Error,
            QString(
              "Value \"%1\" for key \"permissions[%2]\" contains "
              "a value that is not a string \"%3\""
            )
              .arg(
                Oxide::JSON::toJson(permissions),
                QString::number(i),
                Oxide::JSON::toJson(entry)
              )
          );
    ...
    if (app.contains("splash")) {
      ...
      shouldExit if (registrationToMap(app, name).isEmpty()) {
        addError(
          ErrorLevel::Critical, "Unable to convert registration to QVariantMap"
        );
      }
    return errors;
#undef isError
...
#undef shouldExit
  ...
  QVariantMap registrationToMap(const QJsonObject& app, const QString& name) {
    ...
    if (name.isEmpty()) {
      if (
        !app.contains("name") || !app["name"].isString() ||
        app["name"].toString().isEmpty()
      ) {
        return QVariantMap();
      }
  ...
  QList<ValidationError> validateRegistration(QFile* file) {
    ...
    if (!app.isEmpty()) {
      return validateRegistration(
        QFileInfo(file->fileName()).completeBaseName(), app
      );
    }
    QList<ValidationError> errors;
    errors.append(
