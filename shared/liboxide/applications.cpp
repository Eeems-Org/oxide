...
#define addError(_level, _msg)                                                 \
    errors.append(ValidationError{ .level = _level, .msg = _msg });
        ...
        auto isString = [app,
                         &errors,
                         contains](const QString& name, bool required) -> bool {
        ...
        auto isArray =
            ...
            addError(
                level, QString("Key \"%1\" must contain an array").arg(name)
            );
            return false;
        };
        auto isFile =
                ...
                if (!flag.isString()) {
                    addError(
                        ...
                        QString(
                            ...
                            .arg(
                                flagsJson, Oxide::JSON::toJson(flag.toVariant())
                            )
                    );
                    continue;
                }
...
#undef shouldExit
    ...
    QVariantMap registrationToMap(const QJsonObject& app, const QString& name)
    {
        ...
        if (app.contains("workingDirectory")) {
            properties.insert(
                "workingDirectory", app["workingDirectory"].toString()
            );
        }
