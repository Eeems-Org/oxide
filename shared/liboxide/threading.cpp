...
namespace Oxide {
    ...
    void dispatchToThread(QThread* thread, std::function<void()> callback)
    {
        ...
        bool finished = false;
        QObject::connect(
            timer, &QTimer::timeout, thread, [callback, thread, &finished] {
        ...
        QMetaObject::invokeMethod(
            timer, "start", Qt::BlockingQueuedConnection, Q_ARG(int, 0)
        );
    ...
    void runLater(QThread* thread, std::function<void()> callback)
    {
        ...
        QMetaObject::invokeMethod(
            timer, "start", Qt::QueuedConnection, Q_ARG(int, 0)
        );
    }
    ...
    void runInEventLoop(std::function<void(std::function<void()>)> callback)
    {
        ...
        QObject::connect(
            &timer, &QTimer::timeout, qApp, [&loop, &timer, callback] {
                if (!loop.isRunning()) {
                    return;
                }
                timer.stop();
