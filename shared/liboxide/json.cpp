...
qIsNumericType(uint tp) {
  static const qulonglong numericTypeBits =
    ...
    Q_UINT64_C(1) << QMetaType::SChar | Q_UINT64_C(1) << QMetaType::UChar |
    Q_UINT64_C(1) << QMetaType::Short | Q_UINT64_C(1) << QMetaType::UShort |
    Q_UINT64_C(1) << QMetaType::Int | Q_UINT64_C(1) << QMetaType::UInt |
    Q_UINT64_C(1) << QMetaType::Long | Q_UINT64_C(1) << QMetaType::ULong |
    Q_UINT64_C(1) << QMetaType::LongLong |
    Q_UINT64_C(1) << QMetaType::ULongLong;
  return tp < (CHAR_BIT * sizeof numericTypeBits)
           ? numericTypeBits & (Q_UINT64_C(1) << tp)
           : false;
}
...
namespace Oxide::JSON {
  QVariant decodeDBusArgument(const QDBusArgument& arg) {
    ...
    if (
      type == QDBusArgument::BasicType || type == QDBusArgument::VariantType
    ) {
