...
namespace Oxide::Sentry {
  ...
  /*!
   * \brief Get the machine identifier of the device using
   ...
   * \param autoSessionTracking If automatic session tracking should be
   * enabled
   */
  LIBOXIDE_EXPORT void
  sentry_init(const char* name, char* argv[], bool autoSessionTracking = true);
  /*!
   ...
   */
  LIBOXIDE_EXPORT void sentry_breadcrumb(
    const char* category,
    const char* message,
   ...
   */
  LIBOXIDE_EXPORT void sentry_transaction(
   ...
   */
  LIBOXIDE_EXPORT Span* start_span(
   ...
   * \param callback Code to run inside the transaction
   */
  LIBOXIDE_EXPORT void sentry_span(
    Transaction* transaction,
    const std::string& operation,
    const std::string& description,
