...
namespace Oxide {
    ...
    void UDev::deviceType(
        ...
        connect(
            singleton(),
            &UDev::event,
            [callback, subsystem, deviceType](const Device& device) {
    ...
    QList<UDev::Device> UDev::getDeviceList(const QString& subsystem) {
        ...
        if (udev_enumerate_add_match_subsystem(
                udevEnumeration, subsystem.toUtf8().constData()
            ) < 0) {
        ...
        struct udev_list_entry* udevDeviceList =
        ...
        if (udevDeviceList != nullptr) {
            ...
            udev_list_entry_foreach(entry, udevDeviceList) {
                ...
                struct udev_device* udevDevice =
                    udev_device_new_from_syspath(udevLib, path);
    ...
    UDev::ActionType UDev::getActionType(udev_device* udevDevice) {
        ...
        return getActionType(
            QString(devType ? devType : "").trimmed().toUpper()
        );
    }
    ...
    void UDev::monitor() {
        ...
        if (!mon) {
            O_WARNING(
                "UDev::Monitor Unable to listen to UDev: Failed to create "
                "netlink monitor"
            );
        ...
        for (QString subsystem : monitors.keys()) {
            for (QString deviceType : monitors[subsystem]) {
                ...
                if (err < 0) {
                    O_WARNING(
                        "UDev::Monitor Unable to add filter: " << strerror(err)
                    );
                }
            }
        }
        ...
        if (err < 0) {
            O_WARNING(
                "UDev::Monitor Unable to listen to UDev:" << strerror(err)
            );
        ...
        connect(timer, &QTimer::timeout, [this, mon, timer] {
            ...
            if (dev != nullptr) {
                ...
                O_WARNING(
                    "UDev::Monitor error checking event:" << strerror(errno)
                );
            }
            timer->start(30);
        });
