...
_setup()
{
    ...
    for (auto& path :
        ...
        if (item.hasProperty("present") && !item.intProperty("present")) {
            O_DEBUG(
                "    Either missing present property, or battery is not present"
            );
            continue;
        }
    ...
    if (deviceType != Oxide::DeviceSettings::Unknown) {
        ...
        for (QString& path : dir.entryList(
                 QDir::Dirs | QDir::NoDotAndDotDot | QDir::Readable
             )) {
            if ((deviceType == Oxide::DeviceSettings::RM1 &&
                 !path.endsWith(".usbphy")) ||
                (deviceType == Oxide::DeviceSettings::RM2 &&
                 !path.startsWith("usbphy"))) {
                continue;
            }
            O_DEBUG(("  Checking " + path + "...").toStdString().c_str());
            SysObject item(dir.path() + "/" + path);
...
namespace Oxide::Power {
    ...
    QList<QString> batteryAlert()
    {
        ...
        for (SysObject battery : *Oxide::Power::batteries()) {
            ...
            if (battery.hasProperty("temp_alert_max") &&
                temp > battery.intProperty("temp_alert_max")) {
                alerts.append(QString("Overheat"));
            }
            if (battery.hasProperty("temp_alert_min") &&
                temp < battery.intProperty("temp_alert_min")) {
    ...
    bool chargerConnected()
    {
        ...
        if (deviceSettings.getDeviceType() ==
            DeviceSettings::DeviceType::Unknown) {
            return false;
        }
