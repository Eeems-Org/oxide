...
namespace Oxide::Applications {
     ...
     */
    LIBOXIDE_EXPORT QVariantMap
    registrationToMap(const QJsonObject& app, const QString& name = "");
    /*!
    ...
    /*!
     * \brief Validate a application registration file and return any errors
     * found
     ...
     */
    LIBOXIDE_EXPORT QString iconDirPath(
        int size,
        const QString& theme = "hicolor",
        const QString& context = "apps"
    );
    /*!
     ...
     */
    LIBOXIDE_EXPORT QString iconPath(
        const QString& name,
        int size,
        const QString& theme = "hicolor",
        const QString& context = "apps"
     ...
     * \param spec Icon name spec using the following format:
     * [theme:][context:]{name}-{size}. e.g. oxide:splash:xochitl-702
     * \return Path to the icon.
