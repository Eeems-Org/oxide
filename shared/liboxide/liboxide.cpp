...
namespace Oxide {
    ...
    execute(const QString& program, const QStringList& args, bool readStderr)
    {
        ...
        p.setProcessChannelMode(
            readStderr ? QProcess::MergedChannels : QProcess::SeparateChannels
        );
    ...
    QList<pid_t> lsof(const QString& path)
    {
        ...
        auto processes = directory.entryInfoList(
        ...
        for (QFileInfo fi : processes) {
            ...
            auto fds = fd_directory.entryInfoList(
    ...
    uid_t getUID(const QString& name)
    {
        ...
        auto status = getpwnam_r(
            name.toStdString().c_str(), &user, buffer, sizeof(buffer), &result
        );
        if (status != 0) {
            throw std::runtime_error(QString("Failed to get user: %1")
                                         .arg(std::strerror(status))
                                         .toStdString());
        }
        if (result == NULL) {
            throw std::runtime_error(
                "Invalid user name: " + name.toStdString()
            );
        }
    ...
    gid_t getGID(const QString& name)
    {
        ...
        struct group grp;
        struct group* result;
        auto status = getgrnam_r(
            name.toStdString().c_str(), &grp, buffer, sizeof(buffer), &result
        );
        if (status != 0) {
            throw std::runtime_error(QString("Failed to get group: %1")
                                         .arg(std::strerror(status))
                                         .toStdString());
        }
        if (result == NULL) {
            throw std::runtime_error(
                "Invalid group name: " + name.toStdString()
            );
        }
