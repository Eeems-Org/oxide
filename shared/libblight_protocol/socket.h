...
namespace BlightProtocol {
    ...
    LIBBLIGHT_PROTOCOL_EXPORT bool send_blocking(
        int fd, const blight_data_t data, ssize_t size
    );
    /*!
 ...
 * \note The caller is responsible for deallocating the returned data with \c
 * free
 */
LIBBLIGHT_PROTOCOL_EXPORT int blight_recv(
    int fd, blight_data_t* data, ssize_t size
);
/*!
