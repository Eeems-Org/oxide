...
namespace BlightProtocol {
   ...
   * \note The caller is responsible for deallocating the returned data with
   * \c delete[]
   */
   ...
   * \c delete[]
   */
  LIBBLIGHT_PROTOCOL_EXPORT std::optional<blight_data_t>
  recv_blocking(int fd, ssize_t size);
  /*!
  ...
  send_blocking(int fd, const blight_data_t data, ssize_t size);
  /*!
...
blight_recv(int fd, blight_data_t* data, ssize_t size);
/*!
 ...
 */
LIBBLIGHT_PROTOCOL_EXPORT int
blight_recv_blocking(int fd, blight_data_t* data, ssize_t size);
/*!
 * \brief Send data to a socket.
 * \param fd The socket
 * \param data The data to send.
 ...
 */
LIBBLIGHT_PROTOCOL_EXPORT int
blight_send_blocking(int fd, const blight_data_t data, ssize_t size);
/*!
 * \brief Wait until a socket is ready to send data.
 * \param fd The socket
 ...
 */
LIBBLIGHT_PROTOCOL_EXPORT int
blight_wait_for_send(int fd, int timeout);
/*!
 * \brief Wait until a socket is ready for us to receive data.
 * \param fd The socket
 * \param timeout Timeout. -1 will wait forever
 ...
 */
LIBBLIGHT_PROTOCOL_EXPORT int
blight_wait_for_read(int fd, int timeout);
