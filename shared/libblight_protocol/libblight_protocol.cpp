...
extern "C"
{
    ...
    int blight_service_open(blight_bus* bus)
    {
        ...
        if (dfd < 0) {
            _WARN(
                "[blight_service_open::fcntl(...)] Error: %s", std::strerror(e)
            );
            errno = e;
            return -errno;
        }
    ...
    int blight_message_from_socket(int fd, blight_message_t** message)
    {
        ...
        auto m = new blight_message_t{
            .header = blight_header_t{ .type = BlightMessageType::Invalid,
                                       .ackid = 0,
                                       .size = 0 },
            .data = nullptr
        };
        ...
        delete[] maybe.value();
        if (m->header.type == BlightMessageType::Invalid ||
...
struct ack_t
{
    ...
    bool wait(unsigned int timeout)
    {
        ...
        bool res =
            condition.wait_for(lock, timeout * 1ms, [this] { return done; });
        lock.unlock();
        return res;
    }
...
extern "C"
{
    ...
    )
    {
        ...
        if (!send_blocking(
                fd, (blight_data_t)&header, sizeof(blight_header_t)
            )) {
            _WARN(
    ...
    )
    {
        ...
        void* data = mmap(
            NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED_VALIDATE, fd, 0
        );
...
extern "C"
{
    ...
    blight_thread_t* blight_start_connection_thread(int fd)
    {
        ...
        blight_thread_t* thread;
        {
            ...
            std::condition_variable condition;
            thread = new blight_thread_t{ .stop = stop,
                                          .handle = std::thread(
                                              connection_thread,
                                              fd,
                                              stop,
    ...
    int blight_remove_surface(
        int fd,
        BlightProtocol::blight_surface_id_t identifier
    )
    {
        blight_data_t response = nullptr;
        int res = blight_send_message(
        ...
        if (res < 0) {
            _WARN(
                "[blight_remove_surface(...)] Error: %s", std::strerror(errno)
            );
        }
        if (response != nullptr) {
    ...
    int blight_list_surfaces(int fd, blight_surface_id_list_t** list)
    {
        blight_data_t response;
    ...
    blight_surface_id_t* blight_surface_id_list_data(
    ...
    )
    {
        ...
        blight_packet_repaint_t repaint{ .x = x,
                                         .y = y,
                                         .width = width,
                                         .height = height,
                                         .waveform = waveform,
