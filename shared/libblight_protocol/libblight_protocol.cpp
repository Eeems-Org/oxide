...
void blight_bus_deref(blight_bus* bus) { sd_bus_unref(bus); }
...
blight_message_t* blight_message_from_data(blight_data_t data) {
    ...
    if (message->header.size) {
        ...
        memcpy(
            message->data, &data[sizeof(message->header)], message->header.size
        );
    } else {
...
void blight_message_deref(blight_message_t* message) {
    if (message->data != nullptr) {
        if (!message->header.size) {
            _WARN(
                "Freeing blight_message_t with a data pointer but a size of 0"
            );
        }
        delete[] message->data;
    }
...
int blight_send_message(
    ...
    if (!send_blocking(fd, (blight_data_t)&header, sizeof(blight_header_t))) {
        _WARN(
            "[blight_send_message] Failed to write connection message header: "
            "%s",
            std::strerror(errno)
        );
        return -errno;
    ...
    if (size && !send_blocking(fd, data, size)) {
        _WARN(
            "[blight_send_message] Failed to write connection message data: %s",
            std::strerror(errno)
        );
        return -errno;
    }
...
int blight_remove_surface(
    int fd, BlightProtocol::blight_surface_id_t identifier
) {
    blight_data_t response = nullptr;
    int res = blight_send_message(
        fd,
...
int blight_list_surfaces(int fd, blight_surface_id_list_t** list) {
    ...
    // TODO - validate that res can be evenly divided by blight_surface_id_t and
    // error if not
