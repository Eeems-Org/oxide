...
namespace BlightProtocol {
  ...
  recv(int fd, ssize_t size, unsigned int attempts, unsigned int timeout) {
    ...
    try {
      ...
      _WARN(
        "[BlightProtocol::recv(%d, %d, %d, %d)] Not enough memory to "
        "recieve",
        fd,
        size,
        attempts,
    ...
    // We started getting data, but didn't get all of it, try getting some
    // more
    if (res && res < size) {
      _WARN(
        "Partial read (%d), waiting for the rest of the data (%d)",
        res,
        size - res
      );
  ...
  std::optional<blight_data_t> recv_blocking(int fd, ssize_t size) {
    ...
    try {
      ...
      _WARN(
        "[BlightProtocol::recv_blocking(%d, %d)] Not enough memory to "
        "recieve",
        fd,
        size
      );
  ...
  bool send_blocking(int fd, const blight_data_t data, ssize_t size) {
    ...
    while (total < size) {
      ssize_t res =
        ::send(fd, data + total, size - total, MSG_EOR | MSG_NOSIGNAL);
      // We sent something
      if (res > -1) {
        total += res;
        continue;
...
extern "C" {
int
blight_recv(int fd, BlightProtocol::blight_data_t* data, ssize_t size) {
